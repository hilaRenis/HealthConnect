...
const AppointmentEditPage = () => {
  ...
  const loadAdminData = async () => {
    ...
    const patientProfileId = appt.patientProfileId
      || patientsData.find((p) => p.userId === appt.patientUserId)?.id
      || null;
  ...
  const loadDoctorData = async () => {
    ...
    const patientsData = appt.patientProfileId ? [{
      id: appt.patientProfileId,
      name: appt.patientName,
      userId: appt.patientUserId,
    }] : [];
    const doctorsData = [{ id: appt.doctorUserId, name: appt.doctorName }];
    setPatients(patientsData);
    setDoctors(doctorsData);
    ...
    try {
      if (isDoctor) {
        ...
        await adminService.updateAppointment(
          id,
          {
            patientId: appointment.patientProfileId || appointment.patientUserId,
